...
import {
  ...
  humanizePayrollType,
  statusClass,
} from "@/components/dasboard/manage-payroll/payroll-ui-helpers";
import {
  ...
  requestStatusLabel,
  requestTypeLabel,
} from "@/components/dasboard/manage-requests/request-ui-helpers";
...
const isManagerQueueRun = (run: PayrollRunRow) =>
  run.status !== "RELEASED" &&
  run.status !== "VOIDED" &&
  run.managerDecision === PayrollReviewDecision.PENDING;
  ...
  return {
    ...
    actions: [
      ...
      },
      {
        ...
        description: "Inspect current payroll run progress.",
...
const buildGeneralManagerDashboard = async (
  ...
  return {
    ...
    actions: [
      {
        ...
        description: "Approve or release the next payroll runs.",
    ...
    notes: [
      `${gmQueueCount} payroll run${gmQueueCount === 1 ? "" : "s"} are ready for your final approval.`,
...
const buildManagerDashboard = async (
  ...
  const payrollQueueCount = payrollRuns.filter(isManagerQueueRun).length;
  const attendanceFlaggedCount = attendanceRows.filter(isAttendanceFlagged).length;
  ...
  return {
    ...
    subtitle: buildSubtitle(session),
    summary:
      "Focus on requests, payroll review, and operational issues that need a management decision today.",
    timestampLabel: shortNowLabel(),
    stats: [
      {
      ...
      },
      {
        ...
        description: "Runs still waiting on manager review",
        icon: "receipt",
    ...
    actions: [
      ...
      },
      {
        ...
        description: "Approve or return payroll runs with remarks.",
    ...
    notes: [
      ...
      `${activeEmployeeCount} active employee${activeEmployeeCount === 1 ? "" : "s"} currently roll up into your operating view.`,
    ...
    secondaryPanel: {
      ...
      description: "Recent payroll runs with manager approval progress.",
      emptyText: "No payroll runs are ready for manager review.",
      items: payrollRuns
        .filter((run) => isManagerQueueRun(run) || isOpenPayrollRun(run))
        .slice(0, 6)
        .map((run) =>
          buildPayrollItem("manager", run, "/manager/payroll/review-payroll"),
        ),
      footerHref: "/manager/payroll/review-payroll",
      footerLabel: "Open payroll review",
    },
  };
};
...
const buildClerkDashboard = async (session: DashboardSession): Promise<DashboardData> => {
  const [payrollRuns, deductionRows, draftPayrollCount, releasedPayrollCount] =
    await Promise.all([
      safeData(listPayrollRuns({ limit: 6 }), []),
      safeData(listEmployeeDeductionAssignments({ limit: 12 }), []),
      db.payroll.count({ where: { status: PayrollStatus.DRAFT } }),
      db.payroll.count({ where: { status: PayrollStatus.RELEASED } }),
    ]);

  const openDeductions = deductionRows.filter(
    (row) =>
      row.workflowStatus === EmployeeDeductionWorkflowStatus.DRAFT ||
      row.workflowStatus === EmployeeDeductionWorkflowStatus.REJECTED,
  );
  const approvedDeductions = deductionRows.filter(
    (row) => row.workflowStatus === EmployeeDeductionWorkflowStatus.APPROVED,
  ).length;
  const returnedDeductions = deductionRows.filter(
    (row) => row.workflowStatus === EmployeeDeductionWorkflowStatus.REJECTED,
  ).length;

  return {
    role: "clerk",
    roleLabel: formatRoleLabel("clerk"),
    displayName: buildDisplayName(session),
    subtitle: buildSubtitle(session),
    summary:
      "Stay close to payroll preparation and deduction drafts so operations keep moving without rework.",
    timestampLabel: shortNowLabel(),
    stats: [
      {
        label: "Draft Payroll Runs",
        value: toCompactNumber(draftPayrollCount),
        description: "Runs still being prepared or corrected",
        icon: "receipt",
        tone: draftPayrollCount > 0 ? "warning" : "success",
      },
      {
        label: "Released Payroll Runs",
        value: toCompactNumber(releasedPayrollCount),
        description: "Runs already released to employees",
        icon: "banknote",
        tone: "success",
      },
      {
        label: "Open Deduction Drafts",
        value: toCompactNumber(
          deductionRows.filter(
            (row) => row.workflowStatus === EmployeeDeductionWorkflowStatus.DRAFT,
          ).length,
        ),
        description: "Drafts still waiting on manager review",
        icon: "coins",
        tone: "warning",
      },
      {
        label: "Returned Drafts",
        value: toCompactNumber(returnedDeductions),
        description: "Deduction drafts returned for revision",
        icon: "alert",
        tone: returnedDeductions > 0 ? "warning" : "info",
      },
    ],
    actions: [
      {
        title: "Generate Payroll",
        description: "Start or revise draft payroll runs.",
        href: "/clerk/payroll/generate-payroll",
        icon: "receipt",
        badge: draftPayrollCount > 0 ? `${draftPayrollCount} drafts` : undefined,
      },
      {
        title: "Payroll History",
        description: "Review the latest payroll activity and outcomes.",
        href: "/clerk/payroll/payroll-history",
        icon: "banknote",
      },
      {
        title: "Create Deduction Draft",
        description: "Prepare a new deduction draft for review.",
        href: "/clerk/deductions/add",
        icon: "coins",
      },
      {
        title: "My Deduction Drafts",
        description: "Track open, approved, and returned deduction work.",
        href: "/clerk/deductions",
        icon: "file",
        badge: openDeductions.length > 0 ? `${openDeductions.length} open` : undefined,
      },
    ],
    notes: [
      `${draftPayrollCount} payroll run${draftPayrollCount === 1 ? "" : "s"} are still in draft.`,
      `${openDeductions.length} deduction assignment${openDeductions.length === 1 ? "" : "s"} still need follow-through.`,
      `${approvedDeductions} deduction draft${approvedDeductions === 1 ? "" : "s"} were already approved.`,
    ],
    primaryPanel: {
      title: "Payroll Activity",
      description: "The latest payroll runs visible from the clerk queue.",
      emptyText: "No payroll runs are available yet.",
      items: payrollRuns.map((run) =>
        buildPayrollItem("clerk", run, "/clerk/payroll/payroll-history"),
      ),
      footerHref: "/clerk/payroll/payroll-history",
      footerLabel: "Open payroll history",
    },
    secondaryPanel: {
      title: "Deduction Drafts",
      description: "Your current open work plus the latest returned items.",
      emptyText: "No deduction drafts are assigned to you yet.",
      items: sortByNewest(
        openDeductions.length > 0 ? openDeductions : deductionRows,
        (row) => row.updatedAt,
      )
        .slice(0, 6)
        .map((row) =>
          buildDeductionItem(
            ...
            row.workflowStatus === EmployeeDeductionWorkflowStatus.APPROVED
              ? "/clerk/deductions"
              : `/clerk/deductions/add?assignmentId=${row.id}`,
          ),
        ),
      footerHref: "/clerk/deductions",
      footerLabel: "Open deduction drafts",
    },
  };
};
...
export async function loadRoleDashboardData(
  ...
  switch (role) {
    ...
    case "clerk":
      return buildClerkDashboard(session);
    case "employee":
