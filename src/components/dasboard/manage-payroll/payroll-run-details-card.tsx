...
type ApprovalStep = {
  key: "CLERK" | "MANAGER" | "GENERAL_MANAGER";
  label: string;
  ...
  const steps: ApprovalStep[] = [
    {
      ...
      label: "Clerk",
      state: "DONE",
      description: `Drafted ${formatDate(run.generatedAt)}`,
    },
    {
      key: "MANAGER",
      label: "Manager",
      ...
      description: managerRejected
        ? "Rejected by manager"
        : managerApproved
          ? `Approved ${formatDate(run.managerReviewedAt)}`
      ...
      description: managerRejected
        ...
        : gmRejected
          ...
          : gmApproved
            ...
            : managerApproved
              ? "Awaiting GM review"
              : "Blocked until manager approves",
    },
  ];

  const completedCount =
    1 + (managerApproved ? 1 : 0) + (managerApproved && gmApproved ? 1 : 0);
...
const PayrollRunDetailsCard = ({
  ...
  return (
    <>
      <Card className="rounded-[32px] border border-border/70 shadow-sm">
        ...
        <CardContent className="space-y-4">
          ...
          {!loading && !error && run ? (
            <>
              <div className="space-y-6 rounded-[28px] border border-border/70 bg-gradient-to-br from-background via-background to-muted/15 p-5 sm:p-6">
              ...
              {approvalFlow ? (
                <div className="space-y-3 rounded-[24px] border border-border/70 bg-background/85 p-4">
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-xs uppercase tracking-wide text-muted-foreground">
                      Approval Progress (Clerk → Manager → General Manager)
                    </p>
              ...
              <div className="overflow-x-auto rounded-[28px] border border-border/70 bg-background/85">
                <Table>
                  <TableHeader>
                    <TableRow>
