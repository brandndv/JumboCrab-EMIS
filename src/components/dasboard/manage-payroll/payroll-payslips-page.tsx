...
const PayslipReceipt = ({ payslip }: { payslip: PayrollPayslipDetail }) => (
  <section className="rounded-[28px] border border-border/70 bg-gradient-to-b from-background to-muted/10 shadow-sm">
...
const EmployeePayslipDetail = ({
  ...
  <div className="space-y-8 rounded-[32px] border border-border/70 bg-gradient-to-br from-background via-background to-muted/20 p-6 shadow-sm sm:p-7">
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
      ...
      <aside className="space-y-4">
        <section className="rounded-[24px] border border-border/70 bg-background/85 p-5">
...
const PayrollPayslipsPage = ({
  ...
  return (
    <div className="space-y-6 px-4 py-6 sm:px-6 lg:px-8 xl:px-10">
      ...
      {isEmployeeView ? (
        <section className="rounded-[32px] border border-border/70 bg-gradient-to-br from-background via-background to-muted/20 p-6 shadow-sm sm:p-7">
      ...
      <Card
        className={cn(
          "shadow-sm",
          isEmployeeView && "rounded-[28px] border-border/70",
        )}
      >
          ...
          {!loadingRows && !rowsError && isEmployeeHistoryView ? (
            <div className="space-y-4">
              {employeeHistoryGroups.map((group) => (
                <section
                  key={group.label}
                  className="space-y-3 rounded-[24px] border border-border/70 bg-muted/10 p-4"
                >
      ...
      {!isEmployeeHistoryView ? (
        <Card
          className={cn(
            ...
            isEmployeeView &&
              "rounded-[32px] border-none bg-transparent shadow-none",
          )}
        >
          {!isEmployeeView ? (
      ...
      {isEmployeeHistoryView ? (
        ...
        >
          <DialogContent className="max-h-[90vh] overflow-y-auto border-none bg-transparent p-0 shadow-none sm:max-w-5xl">
            ...
            {!loadingDetail && detailError ? (
              <div className="rounded-[32px] border border-destructive/30 bg-destructive/5 p-6 text-sm text-destructive shadow-sm">
                {detailError}
              </div>
            ) : null}
            {!loadingDetail && !detailError && selectedPayslip ? (
